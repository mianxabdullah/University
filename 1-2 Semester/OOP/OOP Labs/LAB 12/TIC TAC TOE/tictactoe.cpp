...
class Player  // Abstract class,stores mark of player 
{
...
{                   // 4  5  6
    ...
    {                                                                                           
        char m = arr[index - 1];  //Get the current value at the position (1-based index).                 
        cout << "Index:" << index << '\t';
        if (m >= '1' && m <= '9') //Check if the spot is empty (still has its initial number) 
        {
            arr[index - 1] = mark; //If free, place the player’s mark.
            cout << "Mark: " << arr[index - 1] << '\n';
            return true;  //success (means cell was empty and mark is placed)
        }
        return false;
    }
    friend class Game;
...
public:
    void init() //→ Initializes the board by filling arr with characters '1' to '9'.
    {            //(49 is ASCII for '1', so i + 49 gives '1' … '9').
...
public:
    ...
    int getMove()  //→ Implements the pure virtual function from Player.
    {
        int index;
...
protected:
    ...
    int BestMove(char arr[9])                         // pick a line to extend (1 of mine + 2 empties)
    {
        for (int i = 0; i < 8; i++)                   // examine each line
        {
            int MyCount = 0, EmptYCount = 0, EmptyIndex = -1; // counters and last seen empty index
            for (int j = 0; j < 3; j++)               // walk the 3 cells in this line
            {
                int index = lines[i][j];              // concrete board index
                if (arr[index] == getMark())          // if it's my mark
                    MyCount++;                        // count it
                else if (arr[index] >= '1' && arr[index] <= '9') // if the cell is still a number -> empty
                {
                    EmptYCount++;                     // count empties
                    EmptyIndex = index;               // remember the (latest) empty slot
                }
            }
            if (MyCount == 1 && EmptYCount == 2)      // exactly one of mine and two empties
                return EmptyIndex + 1;                // take an empty from this promising line
        }
        return -1;                                    // no such line found
    }
